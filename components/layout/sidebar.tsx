...
export function AppSidebar({ open, onClose, session }: AppSidebarProps) {
  ...
  const sidebarContent = (
    <aside
      ...
      className="flex h-full w-64 flex-col border-e border-neutral-200 bg-neutral-50 dark:border-neutral-800 dark:bg-black"
    >
      ...
      <div className="flex h-14 flex-shrink-0 items-center justify-between px-4">
        ...
        <button
          type="button"
          onClick={onClose}
          className="hidden rounded-lg p-1.5 text-neutral-500 transition-colors hover:bg-neutral-200 md:flex dark:hover:bg-neutral-800"
      ...
      <div className="space-y-2 px-3 pt-2 pb-2">
        <Link
          ...
          className="flex w-full items-center gap-3 rounded-xl px-3 py-2 text-sm font-medium text-black transition-colors hover:bg-neutral-200/60 dark:text-white dark:hover:bg-neutral-800/60"
        >
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full rounded-xl border-transparent bg-neutral-200/50 py-2 ps-9 pe-3 text-sm text-black transition-colors placeholder:text-neutral-400 hover:bg-neutral-200/80 focus:border-transparent focus:ring-0 focus:outline-none focus-visible:outline-none dark:bg-neutral-800/50 dark:text-white dark:hover:bg-neutral-800/80"
          />
        </div>
      </div>

      {/* Conversation history */}
      <div className="flex-1 scrollbar-thin scrollbar-thumb-neutral-300 scrollbar-track-transparent overflow-y-auto px-2 pt-2 pb-4 dark:scrollbar-thumb-neutral-700">
        ...
        <ul className="space-y-0.5">
          {filtered.map((c) => (
            <li key={c.id} className="group relative flex">
              <Link
                ...
                className={cn(
                  ...
                  pathname.endsWith(`/chat/${c.id}`)
                    ...
                    : "text-black hover:bg-neutral-200/60 dark:text-white dark:hover:bg-neutral-800/50",
                )}
              >
                {c.title}
              </Link>
              ...
              {overflowId === c.id && (
                <div
                  className="absolute end-0 top-full z-20 mt-0.5 w-36 overflow-hidden rounded-xl border border-neutral-200 bg-white py-1 text-xs shadow-lg dark:border-neutral-700 dark:bg-neutral-900"
      ...
      <div className="mt-auto flex flex-col border-t border-neutral-200 p-2 dark:border-neutral-800">
        <div className="mb-2 space-y-0.5 px-1 pt-1">
          {/* Dark mode */}
          <button
            type="button"
            onClick={() => setTheme(isDark ? "light" : "dark")}
            className="flex w-full items-center gap-3 rounded-lg px-2 py-2 text-sm text-black transition-colors hover:bg-neutral-200/60 dark:text-white dark:hover:bg-neutral-800/60"
          >
            {isDark ? (
        ...
        {/* User profile */}
        <div className="border-t border-neutral-200 pt-3 pb-1 dark:border-neutral-800">
          <div className="flex items-center gap-3 px-2">
            {/* Avatar */}
            <div className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full border border-neutral-100 bg-neutral-50 dark:border-neutral-800 dark:bg-neutral-900/30">
      ...
      {/* Settings modal */}
      <SettingsModal open={settingsOpen} onOpenChange={setSettingsOpen} session={session} />
    </aside>
  );
  ...
  return (
    <>
      ...
      {open && <div className="hidden h-full md:flex">{sidebarContent}</div>}
